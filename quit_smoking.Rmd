...
tax_df  %>% 
  ...
  rename(tax = provision_alt_value) %>% 
  plot_geo(locationmode = 'USA-states') %>% 
  add_trace(
    z = ~tax, text = "$ per pack", locations = ~location_abbr,
    color = ~tax, colors = "Blues") %>% 
    layout(title = "Cigarette Tax ($ per Pack) accroding the U.S.",
           geo = g)
```

```{r}
brfss_df =
  read_xpt("./data/LLCP2020.XPT") %>%
  janitor::clean_names()
```
...
tax_change =
  tax_df %>% 
  filter(year == c("2018", "2020"), 
         measure_desc == "Cigarette") %>% 
  group_by(location_abbr) %>% 
  rename(tax = provision_alt_value) %>% 
  select(location_abbr, year, tax) %>% 
  pivot_wider(
    names_from = year,
    values_from = tax
  ) %>% 
  rename(year1 = '2018', year2 = '2020') %>% 
  unnest(year1, year2) %>% 
  mutate(diff = year2 - year1) %>% view()
