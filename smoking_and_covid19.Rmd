...
## Plot make no sense, since smoking lead to weakened immune system. Could be used for regression analysis for quit smoke as a main factor

```{r}
immune_weaken_df =
  adult_smoke %>%
  filter(immune_weaken %in% c("yes", "no")) %>%
  mutate(immune_weaken = fct_relevel(immune_weaken, "yes", "no")) %>%
  group_by(immune_weaken, smoke) %>%
  summarize(n = n()) %>%
